...
/**
 * This class is for Aggressive Strategy, implements the Strategy interface.
 * 
 * @author Piyush
 */
public class AggressiveStrategy implements Strategy {
	...
	/**
	 * This method finds the strongest country of a player which is the country with
	 * highest no. of armies out of all the countries which can be attacked by the
	 * other players.
	 * 
	 * @param map        Map Object
	...
	/**
	 * This method informs whether an attack is possible on a country or not.
	 * 
	 * @param map        Map Object
	 * @param listPlayer List of Players
	 * @param P          Current Player
			...
			while (true) {
				...
				for (int i = 0; i < strong.getNeighbors().size(); i++) {
					...
					if (!strong.getOwner().equals(neighbor.getOwner())) {
						...
						P.setActions("Attacking country: " + toCountry.getName() + " from country :"
								+ fromCountry.getName());
						...
						if (toCountry.getNoOfArmies() == 0) { // attacker has conquered the defending country.
							...
							P.setActions("Moving armies" + (fromCountry.getNoOfArmies() - 1) + " to country: "
									+ toCountry.getName());
		...
		if (flag == 1) {
			...
			if (max > 1) {
				...
				if (max > 1) {
					...
					System.out.println(
							"Cannot fortify as all the neighboring countries of strongest country have only 1 army left");
					P.setEndOfActions(1);
					P.setActions("Fortification finished");
					return;
				}
